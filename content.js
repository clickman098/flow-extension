(function () {
  ...
  /**
   * Returns only the generated-output image elements in the media grid,
   * NOT ingredient/reference images in the prompt bar or sidepanel.
   *
   * Strategy (most-specific to least):
   *   1. Images whose src contains identifiable Google-generated content paths
   *      (lh3.googleusercontent.com or storage.googleapis.com) — these are the
   *      actual Veo/Imagen outputs stored in GCS.
   *   2. Fallback: img[alt="Generated image"] that are NOT inside the prompt
   *      input area / ingredient pill containers.
   *
   * Reference/ingredient images typically live inside:
   *   - [data-slate-editor] subtrees (the prompt bar)
   *   - elements with class names containing "ingredient" or "reference"
   *   - the upload/preview pill area
   */
  function getGeneratedOutputImages() {
    const allGenImgs = Array.from(
      document.querySelectorAll('img[alt="Generated image"]')
    );

    // Phase 1: prefer images with cloud-storage URLs (actual generated outputs)
    const cloudImgs = allGenImgs.filter(img => {
      const src = img.src || "";
      return (
    ...
    if (cloudImgs.length > 0) return cloudImgs;

    // Phase 2: exclude any img nested inside the prompt editor / ingredient area
    return allGenImgs.filter(img => {
      // Exclude images inside the Slate prompt editor
      if (img.closest('[data-slate-editor="true"]')) return false;
      // Exclude images inside known ingredient/pill/upload containers
      if (img.closest('[class*="ingredient"]')) return false;
      if (img.closest('[class*="reference"]')) return false;
      if (img.closest('[class*="upload"]')) return false;
      if (img.closest('[class*="pill"]')) return false;
      // Exclude images inside the prompt bar area (bottom toolbar)
      if (img.closest('[class*="prompt"]')) return false;
      // Exclude tiny thumbnail images (ingredient pills are usually small)
      const rect = img.getBoundingClientRect();
      if (rect.width > 0 && rect.width < 60 && rect.height < 60) return false;
      return true;
    });
  }
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message?.type === "FLOW_GET_TILE_COUNT") {
      const imgs = Array.from(getGeneratedOutputImages());
      sendResponse({
        count: imgs.length,
        srcs: imgs.map(img => img.src).filter(Boolean),
    ...
    if (message?.type === "FLOW_WAIT_GENERATION") {
      ...
      function getNewUrls() {
        const imgs = Array.from(getGeneratedOutputImages());
        return imgs.filter(img => img.src && !beforeSrcs.has(img.src)).map(img => img.src);
      }

      function currentFailCount() { return countFailCards(); }

      function hasNewImage() {
        const imgs = getGeneratedOutputImages();
        if (imgs.length > beforeCount) return true;
        for (const img of imgs) {
          if (img.src && !beforeSrcs.has(img.src)) return true;
        }
        return false;
      }
      ...
      // Poll every 2s — catches failures React renders after node insertion
      const pollInterval = setInterval(function() {
        if (hasNewImage()) finish({ done: true, newUrls: getNewUrls() });
        else if (hasNewFailure()) finish({ failed: true });
      }, 2000);

      // MutationObserver for fast image detection
